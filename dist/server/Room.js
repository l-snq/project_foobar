...
exports.Room = exports.MAP_DIR = void 0;
...
exports.MAP_DIR = (0, path_1.join)(process.cwd(), "maps");
class Room {
    constructor(id, mapConfig, registry) {
        this.id = id;
        this.registry = registry;
        this.pendingMapChange = new Set();
        ...
        if (mapConfig) {
            ...
            this.map = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(exports.MAP_DIR, `${id}.json`), "utf8"));
        }
        if (this.isHome) {
            this.map = {
                ...this.map,
        ...
        this.objectsFile = (0, path_1.join)(exports.MAP_DIR, `${id}.placed.json`);
        this.physics = new physics_1.RoomPhysics(this.map);
        ...
        if (embedded.length > 0) {
            ...
            try {
                const data = JSON.parse((0, fs_1.readFileSync)(this.objectsFile, "utf8"));
    ...
    saveMap() {
        if (this.isHome && this.ownerUserId) {
            (0, db_1.saveHomeMap)(this.ownerUserId, this.map)
                .catch((e) => console.error(`[home:${this.ownerUserId}] Failed to save map:`, e));
            return;
        }
        try {
            (0, fs_1.writeFileSync)((0, path_1.join)(exports.MAP_DIR, `${this.id}.json`), JSON.stringify(this.map, null, 2));
        }
        catch (e) {
    ...
    add(id, ws) {
        ...
        const handshake = { type: "handshake", yourId: id, tick: this.tick, map: this.map };
        ws.send(JSON.stringify(handshake));
        const objList = { type: "objectList", objects: Array.from(this.placedObjects.values()) };
    ...
    handleMessage(id, raw) {
        ...
        if (msg.type === "join") {
            ...
            if (client.userId) {
                Promise.all([
                    (0, db_1.upsertProfile)(client.userId),
                    (0, db_1.loadInventory)(client.userId),
        ...
        if (msg.type === "shoot") {
            const now = Date.now();
            if (now - client.lastShotAt < FIRE_RATE_MS)
                return;
            const state = this.states.get(id);
            if (!state || state.health <= 0)
                return;
            if (client.weapon !== "pistol")
                return;
            if (client.reloadingUntil > now)
                return;
            if (state.ammo <= 0)
                return;
        ...
        if (msg.type === "placeObject") {
            if (this.isHome && client.userId !== this.ownerUserId)
                return;
            if (typeof msg.url !== "string" ||
                !msg.url.startsWith("/uploads/") ||
                (!msg.url.endsWith(".gltf") && !msg.url.endsWith(".glb")))
                return;
            if (msg.scale < 0.1 || msg.scale > 10)
                return;
            if (Math.abs(msg.x) > 40 || Math.abs(msg.z) > 40)
                return;
            const hitboxRadius = Math.max(0.1, Math.min(10, msg.hitboxRadius ?? 1.0));
            const obj = {
                id: (0, crypto_1.randomUUID)(), url: msg.url, placedBy: id,
            ...
            if (client.userId)
                client.pendingXp += xp_1.XP_PER_OBJECT_PLACED;
        }
        if (msg.type === "placeStoreItem") {
            ...
            if (Math.abs(msg.x) > 40 || Math.abs(msg.z) > 40)
                return;
            (0, storeCache_1.getStoreItem)(msg.itemId).then((storeItem) => {
                if (!storeItem) {
                    console.warn(`[placeStoreItem] item ${msg.itemId} not found in store`);
                    return;
                }
                const obj = {
                    id: (0, crypto_1.randomUUID)(), url: storeItem.model_url, placedBy: id,
                    x: msg.x, z: msg.z, rotY: msg.rotY, scale: msg.scale,
                    hitboxShape: msg.hitboxShape === "box" ? "box" : "cylinder",
        ...
        if (msg.type === "deleteObject") {
            if (this.isHome && client.userId !== this.ownerUserId)
                return;
            if (!this.placedObjects.has(msg.id))
                return;
        ...
        if (msg.type === "kickPlayer") {
            ...
            if (!target || target.userId === this.ownerUserId)
                return;
            try {
        ...
        if (msg.type === "invitePlayer") {
            if (!this.isHome || client.userId !== this.ownerUserId)
                return;
            const delivered = this.registry?.deliver(msg.targetName.trim(), { type: "inviteReceived", fromOwnerName: client.name, homeRoomId: this.id });
            if (!delivered) {
                client.ws.send(JSON.stringify({ type: "inviteError", reason: `"${msg.targetName}" is not online` }));
            }
        }
    ...
    update() {
        ...
        for (const [id, client] of this.clients) {
            ...
            if (client.userId)
                client.pendingXp += xp_1.IDLE_XP_PER_TICK;
        ...
        for (const [id, client] of this.clients) {
            ...
            for (const zone of this.map.waterZones) {
                ...
                if (state.x >= zone.x - halfW && state.x <= zone.x + halfW &&
            ...
            if (!this.pendingMapChange.has(id) && this.tick - client.joinedAtTick >= DOOR_GRACE_TICKS) {
                for (const door of this.map.doors) {
                    ...
                    if (dx * dx + dz * dz < door.triggerRadius * door.triggerRadius) {
                        ...
                        client.ws.send(JSON.stringify({ type: "changeMap", targetMapId: door.targetMapId }));
                        this.remove(id);
                        break;
                    }
                }
        ...
        for (const [pid, proj] of this.projectiles) {
            ...
            const ray = new rapier3d_compat_1.default.Ray({ x: proj.x, y: 0, z: proj.z }, { x: proj.dirX, y: 0, z: proj.dirZ });
            ...
            const hit = this.physics.world.castRay(ray, stepDist + PROJECTILE_RADIUS, true, undefined, undefined, undefined, undefined, (collider) => collider.parent()?.isFixed() ?? false);
            if (hit !== null) {
                this.projectiles.delete(pid);
                continue;
            }
            ...
            for (const [tid, tstate] of this.states) {
                ...
                if (Math.sqrt(dx * dx + dz * dz) < PROJECTILE_RADIUS + physics_1.PLAYER_RADIUS) {
                    this.projectiles.delete(pid);
                    ...
                    if (tstate.health <= 0) {
                        ...
                        if (killerScore)
                            killerScore.kills++;
                        if (shooterClient) {
                            if (shooterClient.userId)
                                shooterClient.pendingXp += xp_1.XP_PER_KILL;
                        ...
                        setTimeout(() => {
                            ...
                            if (b)
                                b.setNextKinematicTranslation({ x: respawnPos.x, y: 0, z: respawnPos.z });
                        }, 3000);
                    }
                    break;
                }
        ...
        if (this.tick % xp_1.XP_FLUSH_TICKS === 0) {
        ...
        this.broadcast({
            ...
            projectiles: Array.from(this.projectiles.values()).map(({ id, ownerId, x, z, dirX, dirZ }) => ({ id, ownerId, x, z, dirX, dirZ })),
            scores: Array.from(this.scores.values()),
        });
    }
    async flushXp() {
        for (const client of this.clients.values()) {
            if (!client.joined || !client.userId || client.pendingXp <= 0)
                continue;
            const xpToFlush = client.pendingXp;
            client.pendingXp = 0;
            try {
                const profile = await (0, db_1.addXpAndCurrency)(client.userId, xpToFlush, 0);
                if (profile.level > client.lastKnownLevel) {
                    const levelsGained = profile.level - client.lastKnownLevel;
                    const currencyAwarded = levelsGained * xp_1.CURRENCY_PER_LEVEL;
                    await (0, db_1.addXpAndCurrency)(client.userId, 0, currencyAwarded);
